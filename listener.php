<?php
	...
	//logIncomingMessage('test.msg',$Xml);
	...
	logIncomingMessage($MessageID.'.msg',$Xml);
	...
	{				
		...
		{
			//echo '<pre>';print_r($objXml->DigitalSignatureIndicator->Value);die;
			...
			if($objXml->DigitalSignatureIndicator->Value == TRUE)
			{
				$blnSiFlagPresent	=	true;
			}
			else
	...
	if($ProcessMessage === TRUE)
	{
		...
		if($blnEpcsArchived === true)
		{
		 ...
		 $blnResult  =   $medDB->AutoExecute(INBOX_MESSAGE_TABLE,$arrRecord,'INSERT');
	...
	function sendEmailAlert($strMessageId,$strErrorDescription,$strMessageType)
	{
       ...
       function logIncomingMessage($strFilename,$strData)
       {
           $strPath        =   'logs/inbox/' .date('Y').'/'.date('m').'/'.date('d').'/';
           ...
           return file_put_contents($strPath.$strFilename,$strData);
       }
