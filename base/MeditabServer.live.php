...
class MeditabServer
{
	...
	public function queryOutBoxMessageStatus($queryOutBoxMessageStatus)
	{
		...
		foreach($rsOutMessageStatus as $arrOutMessageStatus)
		{
			...
			$objOutMessageStatus->ErrorNote = base64_encode($arrOutMessageStatus['error_note']);
	...
	private function mapOutMessageToArray($OutMessage)
	{
		...
		if(RESTRICT_MESSAGE_SUBMISSION_IF_GENERATED_BEFORE_LONG_TIME === TRUE && $OutMessage->RelatedMessageID == "")
		{
			
			$arrIsValidRx = MeditabServer::isValidRxByDate( base64_decode($OutMessage->EDIMessage) );
