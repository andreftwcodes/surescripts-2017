<?php
	...
	if($intSpeakerID < 0)
	{
		echo "Speaker ID not specified.";exit;
	}

	
	$intSleepTime = $intSleepTimeMultiplier = 1; 
	if(isset($argv[2]))
	{
		$intSleepTimeMultiplier = $argv[2];
	}
	$intSleepTime = ($intSleepTimeMultiplier * $intSpeakerID);
	...
	if($intAttempt <= 1)
	{
		...
		$strJobCheckSql = "SELECT job_id, job_start_time, job_termination_time, message_sent_count, last_activity_time, 
							...
							WHERE type ='FIRST_ATTEMPT_POSTER' AND speaker_id = " . $intSpeakerID . " ORDER BY job_id DESC LIMIT 0,1";
...
print_r($rsJob);
	...
	switch($intAttempt)
	{
		...
		CASE '1':
			...
			$strSQL			=	"SELECT * FROM " . OUTBOX_MESSAGE_TABLE . " 
								WHERE message_status = 'Pending' OR message_status = 'Temp_Failed' 
								ORDER BY tran_id ASC LIMIT 0," . SPEAKER_LIMIT_LOCK_MESSAGE_PER_JOB;
	...
	$intTotalRecords = count($rsOutMessages);
	...
	else
	{
		...
		if($intAttempt == 1)
		{
			$arrMessageIDList = array();
			foreach($rsOutMessages as $arrMessage)
			{
				$arrMessageIDList[] = $arrMessage['tran_id'];
			}
			$strMessageIDList = implode(",", $arrMessageIDList);
			$arrLockRecord = array();
			$arrLockRecord['message_status'] = 'Locked';
			
			$arrLockRecord['speaker_id'] = $intSpeakerID;
			$medDB->AutoExecute(OUTBOX_MESSAGE_TABLE, $arrLockRecord, 'UPDATE', 'tran_id IN (' . $strMessageIDList . ') AND speaker_id IS NULL');
			
		}
		
		
		$JOB_ID = logCurrentJob($strJobType, $intSpeakerID);
		...
		foreach($rsOutMessages as $arrOutMessage)
		{
			if($arrOutMessage['message_from'] == 'PRESCRIBER' || $arrOutMessage['message_from'] == 'PHARMACY')
			{
				...
				if( $intLockedRecords < $intTotalRecords )
				{
					$intLockedBySpeakerID = $medDB->GetOne("SELECT speaker_id FROM out_message_transaction WHERE tran_id = " . $arrOutMessage['tran_id']);
					
					
					if( $intLockedBySpeakerID != $intSpeakerID )
					{
	...
	function logCurrentJob($strJobType, $intSpeakerID, $blnNothingToDo = FALSE)
	{
		global $medDB;
		$strTimestamp		=	date('Y-m-d H:i:s');
		if($blnNothingToDo === FALSE)
		{
			$strSql		=	"INSERT INTO " . OUT_MESSAGE_POST_JOB . " (type,speaker_id,job_start_time,last_activity_time) VALUES ('" . $strJobType . "'," . $intSpeakerID . ",'" . $strTimestamp . "','" . $strTimestamp . "')";
		}
		else
		{
